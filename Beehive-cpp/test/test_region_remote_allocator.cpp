...
using namespace Beehive::allocator::remote;
...
int main() {
#ifdef SINGLE_THREAD_TEST
    {
        ...
        std::cout << "available memorty ratio: "
                  << static_cast<double>(available_cnt) /
                         (REMOTE_BUF_SIZE / ALLOC_SIZE)
                  << std::endl;
        ...
        std::cout << "deallocate time: "
                  << static_cast<double>(end - start) / available_cnt
                  << std::endl;
        // check fragments
...
#ifdef MULTI_THREAD_TEST
    {
        ...
        std::cout << "available memory ratio: "
                  << static_cast<double>(global_addrs.size()) /
                         (REMOTE_BUF_SIZE / ALLOC_SIZE)
                  << std::endl;
        size_t available_cnt = global_addrs.size();
        // deallocate
        threads.clear();
        size_t dealloc_per_thread =
            (available_cnt + THREAD_CNT - 1) / THREAD_CNT;
        std::atomic_uint64_t dealloc_time(0);
        auto dealloc_func = [&](size_t base) {
            uint64_t time = 0;
            while (prepare);
            for (size_t idx = base, i = 0;
                 idx < available_cnt && i < dealloc_per_thread; i++, idx++) {
                auto start = get_cycles();
        ...
        std::cout << "allocate time: "
                  << static_cast<double>(allocate_time.load()) / available_cnt
                  << std::endl;
        std::cout << "deallocate time: "
                  << static_cast<double>(dealloc_time.load()) / available_cnt
                  << std::endl;
    }
#endif
    return 0;
