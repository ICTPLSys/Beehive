...
template <typename SrcType, bool Mut, typename LastView, typename TargetType,
          typename Map>
class MapView;
...
public:
    ...
    void for_each(Func &&func) {
        static_cast<Derived *>(this)->template for_each_impl<Func>(
            std::forward<Func>(func));
    }

    void for_each() {
        static_cast<Derived *>(this)->template for_each_impl([]() {});
    }
    ...
    GeneratedVisitView<Visitor> visit(Visitor &&visitor) {
        return GeneratedVisitView<Visitor>(*static_cast<Derived *>(this),
                                           visitor);
    }
    ...
    auto map(Map &&map) {
        ...
        return GeneratedMapView<TargetType, Map>(*static_cast<Derived *>(this),
                                                 map);
    }
};
...
public:
    VisitView(LastView view, Visitor &&visitor)
        : view(view), visitor(std::forward<Visitor>(visitor)) {}
...
template <typename SrcType, bool Mut, typename LastView, typename TargetType,
          typename Map>
class MapView : public View<TargetType, Mut,
                            MapView<SrcType, Mut, LastView, TargetType, Map>> {
...
public:
    MapView(LastView view, Map &&map)
        : view(view), map(std::forward<Map>(map)) {}

    template <typename Func>
    void for_each_impl(Func &&func) {
        view.template for_each([&](SrcReference e) { func(map(e)); });
    }
};
}  // namespace FarView
}  // namespace Beehive
